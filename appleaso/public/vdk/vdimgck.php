...
$config = array(
    ...
    'img_width' => 68,
    'use_boder' => TRUE,
    'font_file' => __DIR__ . '/data/fonts/' . mt_rand(1, 3) . '.ttf',
    'wordlist_file' => __DIR__ . '/data/words/words.txt',
    'filter_type' => 4);
...
if (!echo_validate_image($config)) {
    ...
    $im = @imagecreatefromjpeg(__DIR__ . '/data/vdcode.jpg');
...
function echo_validate_image($config = array())
{
    ...
    $font_file = isset($config['font_file']) ? $config['font_file'] : __DIR__ . '/data/fonts/' . mt_rand(1, 3) . '.ttf';
