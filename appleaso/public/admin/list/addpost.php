...
if(power('sadmin',0) || power('s',$cid,$power,5)) {
	...
	$hidemoudle = getDB() -> all("SELECT * FROM ".tableex('moudle')." where hide<>'1' and ifcreated='1' and ifadmin='1' and cid='$cid'  order by morder asc");
}
$moudle = getDB() -> all("SELECT * FROM ".tableex('moudle')." where hide<>'1' and ifcreated='1' and cid='$cid'$thissql order by morder asc");
$order=0;
$fid=0;
$posttime=time();
...
if(isset($_POST['uuu_addcsarticle'])) {
	...
	for ($i=1;$i<=$addtimes;$i++) {  
		...
		$query = getDB() -> query("INSERT INTO `$articletable` (`posttime`,`cid`,`adminuid`$sql1) VALUES ('$posttime','$cid','$myadminuid'$sql2);");
	}
    getDB() -> commit();
}else {
	$query = getDB() -> query("INSERT INTO `$articletable` (`posttime`,`cid`,`adminuid`$sql1) VALUES ('$posttime','$cid','$myadminuid'$sql2);");
