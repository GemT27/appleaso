...
if (isset($csetting['listfidshow']) && $csetting['listfidshow'] == 1) {
    $fidmoudleinfo = getDB()->one("SELECT strarray FROM " . tableex('moudle') . " where cid='$cid' and mname='fid'$thissql limit 1");
    if ($fidmoudleinfo) {
...
if (isset($_GET['moudleid']) && isset($_GET['fid'])) {
    ...
    $fid = intval($_GET['fid']);
    $ifhavemoudle = getDB()->fetchcount("SELECT id FROM " . tableex('moudle') . " where cid='$cid' and mname='$moudleid'$thissql limit 1");
...
//不是管理员则不要管理员字段
$query = getDB()->query("SELECT * FROM " . tableex('moudle') . " where hide<>'1' and cid='$cid' and ifcreated='1' and ifshow='1'$thissql order by morder asc");
...
<div id="UMain">
    ...
    <div id="mainBox">
        <?php
        if (power('alevel')>=2 && z("开发模式")) {
            echo("<span>栏目cid:" . $cid . "</span>");
        }
        ?>
        <h3>
            <?php
            if (power('s', $cid, $power, 4)) {
                if (power('s', $cid, $power, 5)) {
                    $cidstrcount = getDB()->fetchcount("SELECT count(id) FROM " . tableex('str') . " where strcid='$cid';");
                } else {
                    $cidstrcount = getDB()->fetchcount("SELECT count(id) FROM " . tableex('str') . " where strcid='$cid' and ifadmin='0';");
                }
                if ($cidstrcount > 0) {
            ...
            if (count($where['searchcolumn']) > 1) {
                ...
                <div class="searcharea">
                    <form name="filter" action="?do=list" method="GET">
                        ...
                        <input type="text" class="inputtext" style="width:180px" name="keyword" value="<?php if ($keyword <> '') {
                            echo(trim(htmlspecialchars($_GET['keyword'])));
                        } ?>" placeholder="">
                            <?php
        ...
        <div id="list">
            ...
            <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableBasic">
                <tr>
                    <th width="22" align="center">ID</th>
                    <?php
                        if ($tablecreated) {
                    ...
                    if (isset($csetting['listfidshow']) && $csetting['listfidshow'] == 1) {
                        ...
                        if (isset($fid) && $fid > 0) {
                            $fidwhere = array();
                            if (!empty($articletable)) {
                                $fidwhere['tablename'] = $articletable;
                            }
                            $fidwhere['cid'] = $cid;
...
//                        $where['pagesize'] = 999;
                ...
                if (isset($csetting['articletable'])) {
                    $where['tablename'] = $csetting['articletable'];
                } else {
                    die('栏目尚未配置完成');
                }
                if (!empty($articletable)) {
                    $where['tablename'] = $articletable;
                }
                if (power('sadmin', 0) || power('s', $cid, $power, 5)) {
                } else {
                    if (isset($csetting['listadminuid']) && $csetting['listadminuid'] == 1) {
                        $where['where']['adminuid'] = $myadminuid;
                    }
                }//普通用户只能编辑自己的文章
                                ...
                                if ($thismoudle['mkind'] == 5) {
                                    ...
                                    if ($article[$thismoudle['mname']] > 0) {
                                        if (!isset($strarray[$thismoudle['mname']]['list'][$article[$thismoudle['mname']]])) {
                                            $query = getDB()->query("SELECT id," . $strarray[$thismoudle['mname']]['mname'] . " FROM " . $strarray[$thismoudle['mname']]['tablename'] . " where id='" . $article[$thismoudle['mname']] . "' limit 1;");
                                            if ($thislink = getDB()->fetchone($query)) {
                                    ...
                                    if (is_array($thisstrarray) && count($thisstrarray) > 0) {
                                        foreach ($thisstrarray as $thisvalue) {
                                            if (!isset($strarray[$thismoudle['mname']]['list'][$thisvalue])) {
                                                $query = getDB()->query("SELECT id," . $strarray[$thismoudle['mname']]['mname'] . " FROM " . $strarray[$thismoudle['mname']]['tablename'] . " where id='$thisvalue' limit 1;");
                                                if ($thislink = getDB()->fetchone($query)) {
if (empty($thislink[$strarray[$thismoudle['mname']]['mname']])) {
                                                            $thislink[$strarray[$thismoudle['mname']]['mname']] = '无标题';
                                                        }
...
if (empty($thisstrarrayarticle['title'])) {
						    ...
						    echo('<span class="allarticleschose_adminlist"><a href="?do=list_edit&cid=' . $thisvalue[0] . '&id=' . $thisvalue[1] . '" target="_blank">' . $thisstrarrayarticle['title'] . '</a></span>');
                                    ...
                                    if (empty($article[$thismoudle['mname']])) {
                                        ...
                                        if (isset($strarray[$thismoudle['mname']]['list'][$article[$thismoudle['mname']]])) {
                                            ...
                                            $query = getDB()->one("SELECT id," . $strarray[$thismoudle['mname']]['mname'] . " FROM " . $strarray[$thismoudle['mname']]['tablename'] . " where id='" . $article[$thismoudle['mname']] . "' limit 1;");
                                            if ($query) {
                                ...
                                } elseif ($thismoudle['mkind'] == 10 || $thismoudle['mkind'] == 15) {//复选框
                                    if (!isset($strarray[$thismoudle['mname']])) {
                                        $thismoudlesetting = explode('|', $thismoudle['strarray']);
                                        if (count($thismoudlesetting) <> 2) {
                                            echo('<td>');
                                    ...
                                    if (count($inputvalues) > 0) {
                                        foreach ($inputvalues as $val) {
                                            ...
                                            if (isset($strarray[$thismoudle['mname']]['list'][$val])) {
                                                ...
                                                $query = getDB()->one("SELECT id," . $strarray[$thismoudle['mname']]['mname'] . " FROM " . $strarray[$thismoudle['mname']]['tablename'] . " where id='$val' limit 1;");
                                                if ($query) {
                        ...
                        if (isset($csetting['listshowadminname']) && $csetting['listshowadminname'] == 1) {
                            ...
                            if (isset($article['adminuid'])) {
                                if (!isset($strarray['adminuid'][$article['adminuid']])) {
                                    $strarray['adminuid'][$article['adminuid']] = getDB()->fetchcount("SELECT nickname FROM " . tableex('admin') . " where id='" . $article['adminuid'] . "';");
                                }
                ...
                <div class="action">
                   <div style="display: none"> 已选 <a class="chose_article_count">&nbsp;&nbsp;0&nbsp;&nbsp;</a> 项</div>
