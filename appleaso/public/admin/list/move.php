...
if (!isset($csetting['listarticlemove']) || $csetting['listarticlemove'] == 0) {
    adminmsg('', '禁止转移');
}
...
if ($targetlink['ifshowadmin'] == 0) {
    adminmsg('', '目标栏目已经禁用');
}
$targetcsetting = json_decode($targetlink['csetting'], 1);
if (!isset($targetcsetting['listarticlemove']) || $targetcsetting['listarticlemove'] == 0) {
    adminmsg('', '目标栏目未开启文章转移');
}
if (!isset($targetcsetting['articletable'])) {
    adminmsg('', '目标栏目表名未知');
}
...
foreach ($ids as $value) {
    $value = intval($value);
    $query = getDB()->query("UPDATE $articletable SET cid='$targetcid' WHERE id='$value' and cid='$cid'$delsql");
    if ($query) {
