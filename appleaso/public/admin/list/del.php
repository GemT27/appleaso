...
if (stripos($id, '_') === false) {
    ...
    $thiscid = getDB()->fetchcount("SELECT cid FROM $articletable where id='$id' limit 1");
    //禁止删除
    ...
    if ($thiscid && power('s', $thiscid, $power)) {//有权限
        ...
        $query = getDB()->query("delete from `$articletable` WHERE id='$id' and cid='$thiscid'$delsql");
        if ($query) {
            $msg = "删除成功";
    ...
    foreach ($ids as $value) {
        ...
        $thisid = $value;
        run_admin_hook($cid, 'delete');
        $query = getDB()->query("delete from `$articletable` WHERE id='$value' and cid='$cid'$delsql");
        if ($query) {
