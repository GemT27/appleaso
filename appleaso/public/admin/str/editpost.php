...
foreach ($_POST as $key => $value) 
{
	...
	if(isset($poststr[1]) && is_numeric($poststr[1])) {
		...
		$query = getDB() -> query("SELECT * FROM ".tableex('str')." where id='$thisid' and inputkind>0;");
		...
		if($link) {
			...
			$query = getDB() -> query("UPDATE ".tableex('str')." SET strvalue='$value' WHERE strname='".$strname."' and strcid='$cid';");
			if($query) {
				if($link['ifbind']==1) {//如果该值绑定到栏目,前台链接栏目,修改栏目value
					$query = getDB() -> query("UPDATE ".tableex('channel')." SET cvalue='$value' WHERE cid='$cid'");
				}
