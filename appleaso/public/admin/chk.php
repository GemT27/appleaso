...
if (!defined('loginpage')) {
    ...
    $query = getDB()->query("SELECT * FROM " . tableex('admin') . " where username='" . $ucms_admin . "' limit 1");
    $mjjinfo = getDB()->fetchone($query);
...
function newtoken($input = 1)
{
    ...
    if ($input == 1) {
        echo('<input type="hidden" name="qapp_token" value="' . $token . '">');
    } elseif ($input == 2) {
        echo('qapp_token=' . $token);
    } elseif ($input == 3) {
        Return 'qapp_token=' . $token;
    } elseif ($input == 10) {
        Return true;
    } else {
        Return $token;
...
function checktoken()
{
    if (isset($_POST['qapp_token'])) {
        $token = $_POST['qapp_token'];
    } elseif (isset($_GET['qapp_token'])) {
        $token = $_GET['qapp_token'];
    } else {
...
function setadminname($value,$nickname)
{
    setcookie('admin_' . cookiehash, $value, time() + 604800, "/");
    setcookie('nick_' . cookiehash, $nickname, time() + 604800, "/");
}
...
function setadminpsd($value)
{
    ...
    setcookie('psw_' . cookiehash, $value, time() + 604800, "/");
}
...
$inputkindarray = array(
    ...
    array('id' => '50', 'name' => 'GUID', 'kind' => 'text', 'setting' => 0, 'tips' => '自动生成短GUID'),
);
...
function htmlinput($inputarray)
{
    ...
    if (isset($inputarray['pictips'])) {
        $pictips = $inputarray['pictips'];
    } else {
        $pictips = '';
    }
    if (isset($inputarray['strarray'])) {
        $strarray = $inputarray['strarray'];
    }
    if (isset($inputarray['style'])) {
        $style = $inputarray['style'];
    } else {
        $style = '';
    }
    global $editorarray;
...
function adminchannelscache()
{
    ...
    if (!is_array($channels)) {
        $query = getDB()->query("SELECT * FROM " . tableex('channel') . " order by corder asc");
        $channels = getDB()->fetchall($query);
    }
    Return $channels;
}
...
function getlistcolumn($cid, $setting = array())
{
    ...
    $columnssql[] = 'posttime';
    $columnssql[] = 'adminuid';
    $columns = getDB()->all("SELECT cid,ifshowtemp,hide,mname,ifshow FROM " . tableex('moudle') . " where  cid='$cid' and ifcreated='1' order by morder asc");
...
function getlistsearchcolumn($cid)
{
    $columnsarray = array();
    $columnsarray[] = 'id';
    $columns = getDB()->all("SELECT mname,msetting FROM " . tableex('moudle') . " where  cid='$cid' and ifcreated='1' and hide<>1 order by morder asc");
    foreach ($columns as $key => $val) {
...
function adminchannelcache($cid)
{
    ...
    if (!isset($channel['csetting']['articletable']) || empty($channel['csetting']['articletable'])) {
        Return false;
    }
...
function channeltable($cid, $articletable)
{
    ...
    if ($site_db['database'] == 'sqlitepdo') {
        $query = getDB()->query("CREATE TABLE if not exists [" . $articletable . "] ([id] INTEGER  NOT NULL PRIMARY KEY AUTOINCREMENT,[posttime] INTEGER  NULL,[cid] INTEGER NOT NULL,[adminuid] INTEGER  NULL)");
    } elseif ($site_db['database'] == 'mysqlpdo' || $site_db['database'] == 'mysql') {
        $tableMysqlEngine = 'MyISAM';
        /* if (defined('MysqlEngine') && strtolower(MysqlEngine) == 'myisam') {
             $tableMysqlEngine = 'MyISAM';
         } else {
             $tableMysqlEngine = 'InnoDB';
         }
        */
        $query = getDB()->query("CREATE TABLE if not exists `" . $articletable . "` (`id` int(8) NOT NULL auto_increment,`posttime` int(10) default 0,`cid` int(8) default 0,`adminuid` int(8) default 0,PRIMARY KEY  (`id`)) ENGINE=" . $tableMysqlEngine . " DEFAULT CHARSET=utf8;");
    } else {
        echo('database error');
        Return false;
    }
    if ($site_db['database'] == 'sqlitepdo') {
        $query = getDB()->query("SELECT * FROM sqlite_master WHERE type='table' and name='" . $articletable . "';");
        $articletablelist = getDB()->fetchone($query);
    } elseif ($site_db['database'] == 'mysqlpdo' || $site_db['database'] == 'mysql') {
        $query = getDB()->query("show columns from `$articletable`");
    ...
    $neworder = 1;
    $quickadd = 0;
    $links = getDB()->all("SELECT * FROM " . tableex('moudle') . " where cid='$cid' and hide<>'1' order by morder asc");
    $countmoudle = 0;
        ...
        if (stripos($articletablelist['sql'], '[' . $link['mname'] . ']') > 0 || stripos($articletablelist['sql'], ' ' . $link['mname'] . ' ') > 0) {
            ...
            if ($val) {
                if (isset($val['kind']) && $val['kind'] == 'int') {
                    $query = getDB()->query("alter table $articletable add $mname INTEGER NULL;");
                } elseif (isset($val['kind']) && $val['kind'] == 'varchar') {
                    $query = getDB()->query("alter table $articletable add $mname VARCHAR(250) NULL;");
                } elseif (isset($val['kind']) && $val['kind'] == 'text') {
                    $query = getDB()->query("alter table $articletable add $mname TEXT NULL;");
                } elseif (isset($val['kind'])) {
                    $query = getDB()->query("alter table $articletable add $mname " . $val['kind'] . " NULL;");
                } else {
                    $query = getDB()->query("alter table $articletable add $mname VARCHAR(250) NULL;");
                }
            }
...
function channelrouter()
{
    ...
    $diyurl = getDB()->one("SELECT * FROM " . tableex('str') . " where strname='diyurl' and inputkind=0 limit 1;");
    if ($diyurl) {
    ...
    $channels = getDB()->all("SELECT * FROM " . tableex('channel') . " order by cid asc");
    foreach ($channels as $key => $val) {
        $csetting = json_decode($val['csetting'], 1);
        if ($val['ifshow'] > 0) {
            if (!isset($csetting['channel_domain'])) {
    ...
    if (!$routerquery) {
        $query = getDB()->query("INSERT INTO `" . tableex('str') . "` (`strname`,`strcid`,`strorder`,`inputkind`,`strvalue`,`ifadmin`) VALUES ('router','0','0','0','$allurl','1');");
    } else {
        $query = getDB()->query("UPDATE " . tableex('str') . " SET strvalue='$allurl' WHERE strname='router' and inputkind=0;");
        cachedel('router', 'str');
    }
    if ($query) {
        Return true;
...
function transit_channel_cache()
{
    ...
    foreach ($channels as $key => $val) {
        if ($val['ckind'] == 5) {
            ...
            $query = getDB()->query("UPDATE " . tableex('channel') . " SET cvalue='$value' WHERE cid='" . $val['cid'] . "'");
            if (!$query) {
                Return false;
            }
        }
