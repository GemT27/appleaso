...
if(empty($inputvalue)) {
	...
	$query = getDB() -> query("SELECT id,$mname,fid FROM $thisarticletable where cid='$cid' and fid=0 $thisorder limit 10000;");
	...
	if($inputvalue>0) {
		...
		if($ifson>0) {
			...
			$query = getDB() -> query("SELECT id,$mname,fid FROM $thisarticletable where cid='$cid' and fid='$inputvalue' $thisorder limit 10000;");
			$strarray = getDB() -> fetchall($query);
			foreach ($strarray as $value) 
			{
	...
	while($times<20) {
		...
		$query = getDB() -> query("SELECT id,$mname,fid FROM $thisarticletable where cid='$cid' and fid='$upfid' $thisorder limit 10000;");
		$strarray = getDB() -> fetchall($query);
		if(count($strarray)==0) {
			break;
		}
