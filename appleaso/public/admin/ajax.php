...
if ($_GET['do'] == 'amoudleselect') {
    ...
    if ($_POST['from'] == 'str') {
        $query = getDB()->query("SELECT * FROM " . tableex('str') . " where id='$moudleid';");
    } else {
        $query = getDB()->query("SELECT * FROM " . tableex('moudle') . " where id='$moudleid';");
    }
    ...
    $query = getDB()->query("SELECT id,$mname FROM $thisarticletable where cid='$cid' and $mname like '%$keyword%' $thisorder limit 100;");
...
if ($_GET['do'] == 'amoudleajaxkind') {
    ...
    $selectstr = '';
    $iflive = getDB()->fetchcount("SELECT id FROM $thisarticletable where cid='$cid' and id='$inputvalue' limit 1;");//判断是否存在
    if ($iflive) {
    } else {
        $inputvalue = 0;
    }
    $fid = getDB()->fetchcount("SELECT fid FROM $thisarticletable where cid='$cid' and id='$inputvalue' limit 1;");
    if ($fid) {
    } else {
        $fid = 0;
    }
    if ($inputvalue > 0) {
        $ifson = getDB()->fetchcount("SELECT count(id) FROM $thisarticletable where cid='$cid' and fid='$inputvalue' limit 1;");//判断该id下是否有子分类
        if ($ifson > 0) {
            $selectstr .= '<select name=""><option value="' . $inputvalue . '" selected>无</option>' . "\r\n";
            $query = getDB()->query("SELECT id,$mname,fid FROM $thisarticletable where cid='$cid' and fid='$inputvalue' $thisorder limit 3000;");
            $strarray = getDB()->fetchall($query);
    ...
    while ($times < 20) {
        ...
        $query = getDB()->query("SELECT id,$mname,fid FROM $thisarticletable where cid='$cid' and fid='$upfid' $thisorder limit 3000;");
        $strarray = getDB()->fetchall($query);
        if (count($strarray) == 0) {
            break;
        }
        ...
        $oldfid = $upfid;
        $upfid = getDB()->fetchcount("SELECT fid FROM $thisarticletable where cid='$cid' and id='$upfid' limit 1;");
    }
    echo($selectstr);
...
if ($_GET['do'] == 'strarraylist') {
    ...
    $select = 0;
    $query = getDB()->query("SELECT * FROM " . tableex('moudle') . " where cid='$cid' and hide<>'1' order by morder asc");
    $links = getDB()->fetchall($query);
    if (count($links) == 0) {
...
if ($_GET['do'] == 'listcheckboxchange') {
    ...
    $query = getDB()->query("SELECT * FROM " . tableex('channel') . " where cid='$cid' limit 1");
    $link = getDB()->fetchone($query);
    ...
    $query = getDB()->query("SELECT * FROM $articletable where id='$articleid' and cid='$cid'$delsql limit 1");
    $link = getDB()->fetchone($query);
    if (!$link) {
    ...
    $query = getDB()->query("SELECT * FROM " . tableex('moudle') . " where hide<>'1' and ifcreated='1' and mname='$field' and cid='$cid'$thissql limit 1");
    $moudle = getDB()->fetchone($query);
    if ($moudle) {
        $query = getDB()->query("UPDATE $articletable SET $field='$status' WHERE id='$articleid' and cid='$cid'");
    } else {
