...
if($psd=='') {
	$query = getDB() -> query("UPDATE ".tableex('admin')." SET nickname='$nickname' WHERE username='$thisname'");
	if($query) {
		run_sys_hook("user_mypost","user_myedit_success");
		$msg='修改成功';
	}else {
		$msg='修改失败';
	}
	...
	$query = getDB() -> query("UPDATE ".tableex('admin')." SET nickname='$nickname',psd='$psd' WHERE username='$thisname'");
	if($query) {
        run_sys_hook("user_mypost","user_myedit_success");
		$msg='修改成功,请重新登录';
