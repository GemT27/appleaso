...
if($psd<>$psd1) {
	...
	if($username=='') {
		...
		$pagecount =getDB() -> fetchcount("SELECT count(id) FROM ".tableex('admin')." where username='$username'");
		if($pagecount>0) {
			$msg='已存在该用户名';
		}else {
			$query = getDB() -> query("INSERT INTO `".tableex('admin')."` (`username` ,`nickname`,`psd`,`alevel`,`power`,`kind`) VALUES ('$username','$nickname','$psd','".$quan['alevel']."','$thispower','0');");
			if($query) {

				$msg='添加成功';
                run_sys_hook("user_add_post","user_add_success");
			}else {
				$msg='添加失败';
			}
		}
