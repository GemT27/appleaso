...
$username = getDB() -> fetchcount("SELECT username FROM `".tableex('admin')."` where id='$id' and alevel<='$alevel' limit 1");
if(!$username) {
	adminmsg('','用户不存在',3);
}
$psd=trim($_POST['psd']);
...
//系统中必须有一个超级管理员
if($quan['alevel']<3) {
	$superadmincount = getDB() -> fetchcount("SELECT count(id) FROM `".tableex('admin')."` where id<>'$id' and alevel='3'");
...
$query = getDB() -> query("UPDATE ".tableex('admin')." SET nickname='$nickname',alevel='".$quan['alevel']."',power='$thispower'$sql WHERE id='$id'");
if($query) {
    run_sys_hook("user_edit_post","user_edit_success");
	$msg='修改成功';
}else {
	$msg='修改失败';
}
