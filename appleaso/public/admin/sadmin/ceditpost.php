...
if($ckind==1) {
	...
	$cvalue=$_POST['cvalue'];
	$ifbindstr = getDB() -> fetchcount("SELECT count(id) FROM ".tableex('str')." where strcid='$cid' and ifbind='1'");
	if($ifbindstr<1) {
		$query = getDB() -> query("INSERT INTO `".tableex('str')."` (`strname`,`strcid`,`strorder`,`inputkind`,`strvalue`,`ifbind`) VALUES ('$cname','$cid','1','1','','1');");
	}
	$query = getDB() -> query("UPDATE `".tableex('str')."` SET strvalue='$cvalue' WHERE strcid='$cid' and ifbind='1'");
}elseif($ckind==4) {
...
$query = getDB() -> query("UPDATE ".tableex('channel')." SET 
