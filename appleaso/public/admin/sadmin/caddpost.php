...
if(isset($_POST['multiple_add'])) {
	foreach($_POST['cname'] as $key=>$val) {
		...
		$query = getDB() -> query("INSERT INTO `".tableex('channel')."` (`fid`,`cname`,`ckind`,`corder`,`ifshow`,`ifshownav`,`ifshowadmin`,`ifshowleft`,`csetting`) VALUES ('$fid','$cname','$ckind','$corder','1','1','1','1','$defaultcsetting');");
		if($query) {
			$lastid=getDB()->lastId();
			//如果是链接,创建一个该栏目的str
			if($ckind==3) {
				$query = getDB() -> query("INSERT INTO `".tableex('str')."` (`strname`,`strcid`,`strorder`,`inputkind`,`strvalue`,`ifbind`) VALUES ('网址','$lastid','1','1','','1');");
			}
		}
...
$order = getDB() -> fetchcount("SELECT corder FROM ".tableex('channel')." order by corder desc limit 1");
if($order) {
	$corder=$order+5;
}else {
	$corder=5;
...
$ifshowleft=1;
$query = getDB() -> query("INSERT INTO `".tableex('channel')."` (`fid`,`cname`,`ckind`,`corder`,`ifshow`,`ifshownav`,`ifshowadmin`,`ifshowleft`) VALUES ('$fid','$cname','$ckind','$corder','1','1','1','$ifshowleft');");
if($query) {
	$lastid=getDB()->lastId();
	//修改新的栏目的默认设置
	$defaultcsetting=array();
	...
	if($ckind==1 || $ckind==2) {
		//父栏目域名
		$fidsetting=getDB() -> one("SELECT csetting FROM ".tableex('channel')." where cid='$fid' limit 1");
		if($fidsetting) {
	...
	$defaultcsetting=json_encode($defaultcsetting);
	$query1 = getDB() -> query("UPDATE ".tableex('channel')." SET csetting='$defaultcsetting' WHERE cid='$lastid'");
	if($query1) {
	...
	if($lastid>0)
	{
		...
		if($settingfile==2 && empty($_FILES['txt']['error'])) {
			...
			$fidinfo = getDB() -> one("SELECT * FROM ".tableex('channel')." where cid='$settingfile_copy'");
			if(!$fidinfo) {
				adminmsg('','指定的栏目已不存在');
			}
			$channeldata=array();
			...
			$moudles = getDB() -> all("SELECT * FROM ".tableex('moudle')." where cid='$settingfile_copy' order by morder asc");
			...
			$strs = getDB() -> all("SELECT * FROM ".tableex('str')." where strcid='$settingfile_copy' order by strorder asc");
		...
		if($ckind==1) {
			$query = getDB() -> query("INSERT INTO `".tableex('str')."` (`strname`,`strcid`,`strorder`,`inputkind`,`strvalue`,`ifbind`) VALUES ('内容','$lastid','1','4','','0');");
		}elseif($ckind==3) {
			$query = getDB() -> query("INSERT INTO `".tableex('str')."` (`strname`,`strcid`,`strorder`,`inputkind`,`strvalue`,`ifbind`) VALUES ('网址','$lastid','1','1','','1');");
		}
