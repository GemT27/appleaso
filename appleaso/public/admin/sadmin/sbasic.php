...
<div id="urHere"><em class="homeico"></em>后台管理<b>&gt;</b><strong>栏目配置</strong> </div>   <div id="mainBox">
	...
	if($cid>0) {
		...
		echo('<a style="margin-left:10px" href="?do=sadmin_cout&cid=0&nohtml=1" class="actionBtn">导出系统变量</a>');
	}
	?>
...
<?php
$neworder=0;
	$query = getDB() -> query("SELECT * FROM ".tableex('str')." where strcid='$cid' and inputkind>0 order by strorder asc");
	$linkbasic = getDB() -> fetchall($query);
