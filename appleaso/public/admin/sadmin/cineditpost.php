...
if(isset($_POST['cid'])) {
	...
	if($cid>0) {
		...
		$thischannel = getDB() -> fetchone($query);
		if(!$thischannel) {
			adminmsg('','栏目不存在',1);
		}
	...
	if(isset($channelarray['info']) && isset($_POST['info'])) {
		...
		if(isset($channelarray['info']['fid']) && !empty($channelarray['info']['fid'])) {
			...
			$fidcid = getDB() -> one("SELECT cid FROM ".tableex('channel')." where cname='".$channelarray['info']['fid']."' limit 1");
			if(isset($fidcid['cid'])) {
		...
		$query = getDB() -> update(tableex('channel'),"cid='$cid'",$newchannelinfo);
		if(!$query) {
	...
	if(isset($_POST['moudle']) && isset($channelarray['moudle'])) {
		...
		foreach($channelarray['moudle'] as $thismoudle) {
			if(isset($_POST['moudle'][$thismoudle['id']])) {
				...
				$pagecount = getDB() -> fetchcount("SELECT count(id) FROM ".tableex('moudle')." where cid='$cid' and mname='$thismname';");
				if($pagecount==0) {
					$insetcount++;
					...
					$thisid = getDB() -> insert(tableex('moudle'),$thismoudlearray);
				}
			}
		}
	...
	if(isset($_POST['str']) && isset($channelarray['str'])) {
		foreach($channelarray['str'] as $thisstr) {
			if(isset($_POST['str'][$thisstr['id']])) {
				...
				$pagecount = getDB() -> fetchcount("SELECT count(id) FROM ".tableex('str')." where strcid='$cid' and strname='$thisstrname';");
				if($pagecount==0) {
					$thisstrarray=array();
					...
					$thisid = getDB() -> insert(tableex('str'),$thisstrarray);
				}
			}
		}
	}
