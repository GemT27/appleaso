...
if(isset($_POST['diyurl'])) {
	...
	$allurl = getDB() -> one("SELECT * FROM ".tableex('str')." where strname='diyurl' and inputkind=0 limit 1;");
	if(!$allurl) {
		$query = getDB() -> query("INSERT INTO `".tableex('str')."` (`strname`,`strcid`,`strorder`,`inputkind`,`strvalue`,`ifadmin`) VALUES ('diyurl','0','0','0','$newurl','1');");
	}else {
		$query = getDB() -> query("UPDATE ".tableex('str')." SET strvalue='$newurl' WHERE strname='diyurl' and inputkind=0;");
	}
	if($query) {
		channelrouter();
