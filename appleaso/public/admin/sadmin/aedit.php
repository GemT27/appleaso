...
$query = getDB() -> query("SELECT * FROM ".tableex('moudle')." where id='$id'");
$link = getDB() -> fetchone($query);

$msetting=json_decode($link['msetting'],1);
$cid = $link['cid'];
$cname = getDB() -> fetchcount("SELECT cname FROM ".tableex('channel')." where cid='$cid' limit 1");
?>
<div id="UMain">
  <!-- 当前位置 -->
		...
		<input type="hidden" name="cid" value="<?php echo($link['cid']);?>">
			...
			<tr><td width="10%" align="right">字段描述</td>
				...
				<select name="fid" id="fid">
					...
					<?php
					$query = getDB() -> query("SELECT * FROM ".tableex('moudle')." where cid='$cid' and id<>'$id' order by morder asc;");
					$allmoudle = getDB() -> fetchall($query);
					$sonarray=array();
