...
$showindexcount = getDB() -> fetchcount("SELECT count(id) FROM ".tableex('moudle')." where cid='$cid' and ifshow<>'0';");
$neworder = getDB() -> fetchcount("SELECT morder FROM ".tableex('moudle')." where cid='$cid' order by morder desc limit 1");
if(!$neworder) {
	$neworder=0;
}
$ifshowtemp='';
		...
		if(strlen($_POST['minfo'][$key])>0) {
			...
			if(strlen($mname)<1) {
				...
				if(in_array($mname,$notallowfield)) {
					...
					$pagecount = getDB() -> fetchcount("SELECT count(id) FROM ".tableex('moudle')." where mname='$mname' and cid='$cid'");
					if($pagecount>0) {
						$msg.=$minfo.':已存在该字段 ';
					}else {
						$strdefault='';
						...
						$query = getDB() -> query("INSERT INTO `".tableex('moudle')."` (`mname` ,`minfo`,`mkind`,`morder`,`cid`,`ifshow`,`ifonly`,`strdefault`,`mcontent`,`ifshowtemp`,`hide`,`ifadmin`,`fid`,`msetting`,`ifcreated`) VALUES ('$mname','$minfo','$mkind','$morder','$cid','$ifshow','0','$strdefault','','$ifshowtemp','0','0','','$msetting','0');");
						if($query) {
						}else {
							$msg.=$minfo.':添加失败 ';
						}
...
if(isset($_POST['createdtable']) && $_POST['createdtable']==1) {
	$query = getDB() -> query("SELECT * FROM ".tableex('channel')." where cid='$cid'");
	...
	if($createreturn===true) {
		$query = getDB() -> query("UPDATE ".tableex('moudle')." SET ifcreated='1' WHERE cid='$cid'");
