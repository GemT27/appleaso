...
$pagecount = getDB() -> fetchcount("SELECT count(id) FROM ".tableex('moudle')." where mname='$mname' and cid='$cid' and id<>'$mid'");
if($pagecount>0) {
	adminmsg('','已存在该字段',3);
}
	if(!isset($_POST['fid'])) {
	...
	$oldmname = getDB() -> fetchcount("SELECT mname FROM ".tableex('moudle')." where id='$mid';");
	$query = getDB() -> query("UPDATE ".tableex('moudle')." SET fid='$fid',hide='$hide',ifadmin='$ifadmin',ifonly='$ifonly',ifmore='$ifmore',strdefault='$strdefault',mfunction='$mfunction',mname='$mname',minfo='$minfo',mcontent='$mcontent',mkind='$mkind',morder='$morder',ifshow='$ifshow',ifshowtemp='$ifshowtemp',strarray='$strarray',msetting='$msetting' WHERE id='$mid'");
	if($query) {
		if($oldmname<>$mname) {
			$query = getDB() -> query("UPDATE ".tableex('moudle')." SET ifcreated='0' WHERE id='$mid'");
			$query = getDB() -> query("UPDATE ".tableex('moudle')." SET fid='$mname' WHERE fid='$oldmname' and cid='$cid'");
		}
