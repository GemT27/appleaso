...
if(isset($_GET['cid']) && $_GET['cid']==0) {
	$channeldata=array();
	$strs = getDB() -> all("SELECT * FROM ".tableex('str')." where strcid='0' order by id asc");
	$channeldata['str']=$strs;
	filedownload('系统变量',$channeldata);
}
$link = getDB() -> one("SELECT * FROM ".tableex('channel')." where cid='$cid'");
if(!$link) {
	die('no channel');
}
...
if(isset($_POST['cid'])) {
	...
	if(isset($_POST['info'])) {
		if($link['fid']>0) {
			$fidname = getDB() -> one("SELECT cname FROM ".tableex('channel')." where cid='".$link['fid']."' limit 1");
			if(isset($fidname['cname'])) {$link['fid']=$fidname['cname'];}
		}
		$channeldata['info']=$link;
	}
	if(isset($_POST['moudle'])) {
		$moudles = getDB() -> all("SELECT * FROM ".tableex('moudle')." where cid='$cid' order by morder asc");
	...
	if(isset($_POST['str'])) {
		$strs = getDB() -> all("SELECT * FROM ".tableex('str')." where strcid='$cid' order by strorder asc");
