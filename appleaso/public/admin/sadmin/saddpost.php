...
$neworder = getDB() -> fetchcount("SELECT strorder FROM ".tableex('str')." where strcid='$strcid' order by strorder desc limit 1");
if(!$neworder) {
	$neworder=0;
}
$msg='';
...
if(isset($_POST['strname'])) {
	foreach($_POST['strname'] as $key=>$val) {
		if(!empty($_POST['strname'][$key])) {
			...
			$pagecount = getDB() -> fetchcount("SELECT count(id) FROM ".tableex('str')." where strname='$strname' and strcid='$strcid'");
			if($pagecount>0) {
				$msg.=$strname.':已存在该变量 ';
			}else{
				if(stripos($strname,'.')===false) {
					$query = getDB() -> query("INSERT INTO `".tableex('str')."` (`strname`,`strcid`,`strorder`,`inputkind`,`strvalue`,`ifadmin`) VALUES ('$strname','$strcid','$strorder','$inputkind','','0');");
					if(!$query) {
						$msg.=$strname.':增加失败 ';
					}
				}else {
