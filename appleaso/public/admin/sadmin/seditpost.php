...
$pagecount = getDB() -> fetchcount("SELECT count(id) FROM ".tableex('str')." where strname='$strname' and id<>'$id' and strcid='$strcid' and inputkind>0");
if($pagecount>0) {
	adminmsg('','变量名重复',1);
}
if(isset($_POST['ifbind'])) {
	$ifbind=1;
	$pagecount = getDB() -> fetchcount("SELECT count(id) FROM ".tableex('str')." where strcid='$strcid' and ifbind='1' and id<>'$id' and inputkind>0");
	if($pagecount>0) {
...
$query = getDB() -> query("UPDATE ".tableex('str')." SET strname='$strname',inputkind='$inputkind',strstyle='$strstyle',strorder='$strorder',strtip='$strtip',strarray='$strarray',ifbind='$ifbind',ifadmin='$ifadmin',ssetting='$ssetting' WHERE id='$id'");
