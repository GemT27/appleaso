...
if($articletable=='尚未填写') {
	...
	if($site_db['database']=='sqlitepdo') {
		...
		$query = getDB() -> query("select table_name from information_schema.tables where table_name='$articletable' and TABLE_SCHEMA='".$site_db['dbname']."';");
		$tablenames = getDB() -> fetchone($query);
		if($tablenames) {
			$query = getDB() -> query("show columns from `$articletable`");
			$tablecolumns= getDB() -> fetchall($query);
			$articletablelist['sql']=' ';
...
<?php
$neworder=0;
$query = getDB() -> query("SELECT * FROM ".tableex('moudle')." where cid='$cid' order by morder asc");
...
<tr>
	...
	<?php
		...
		if($val && $val['strfrom']==1) {
			if(count($thissetting)<2) {
				...
				$query = getDB() -> one("SELECT id FROM ".tableex('moudle')." where cid='".$thissetting[0]."' and mname='".$thissetting[1]."' and hide<>'1' limit 1");
				if(!$query) {
	...
	<td align=center>
	<?php
	//var_dump($articletablelist);
	//echo '['.$link['mname'].']';
	//echo stripos($articletablelist['sql'],$link['mname'].'');
	if(stripos($articletablelist['sql'],'['.$link['mname'].']')>0 || stripos($articletablelist['sql'],' '.$link['mname'].' ')>0  || stripos($articletablelist['sql'],'"'.$link['mname'].'"')>0) {
		echo('已建');
		if($link['ifcreated']==0) {
			$query = getDB() -> query("UPDATE ".tableex('moudle')." SET ifcreated='1' WHERE id='".$link['id']."'");
		}
	}else {
		$someonenotadd=1;
		echo('<font color="#FF0000">未建</font>');
		if($link['ifcreated']==1) {
			$query = getDB() -> query("UPDATE ".tableex('moudle')." SET ifcreated='0' WHERE id='".$link['id']."'");
		}
	}
?>
	</td>
