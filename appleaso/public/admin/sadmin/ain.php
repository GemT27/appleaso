...
if(!empty($_FILES['txt']['error']))
{
	...
	foreach($strs as $value) {
		$mname=$value['mname'];
		$pagecount = getDB() -> fetchcount("SELECT count(id) FROM ".tableex('moudle')." where cid='$cid' and mname='$mname';");
		if($pagecount>0) {
			...
			if(is_numeric($value['fid']) && $value['fid']<>0 && $value['fid']<>null) {
				foreach($strs as $values) {
					if($value['fid']==$values['id']) {
						...
						$thisfid = getDB() -> fetchcount("SELECT id FROM ".tableex('moudle')." where cid='$cid' and mname='".$values['mname']."';");
						if($thisfid) {
						}else {
							$thisfid=0;
						}
			...
			$query = getDB() -> query("INSERT INTO `".tableex('moudle')."` (`fid`,`mname` ,`minfo`,`mkind`,`morder`,`cid`,`ifshow`,`ifmore`,`mcontent`,`ifshowtemp`,`hide`,`strarray`,`strdefault`,`mfunction`) 
