...
//初始化站点
if (!file_exists(IncDir . 'sitehash.php')) {
    $sitehash = getCuid();
    $dbhash = getCuid();
    file_put_contents(IncDir . 'sitehash.php', "<?php define('SiteHash','{$sitehash}');define('DBHash','{$dbhash}');");
    copy(SystemRoot . '../data/sys.db', SystemRoot . '../data/' . $dbhash . '.db');
}
require(IncDir . 'sitehash.php');
define('cookiehash', substr(md5(SiteHash), 0, 6));


$site_db = array('database' => 'sqlitepdo', 'dbfile' => SystemRoot . '../data/' . DBHash . '.db');
...
if (SystemDir != "/") {
    $systemDir = SystemDir;
    if (substr(SystemDir, 0, 1) != '/') {
        $systemDir = '/' . $systemDir;
    }
    $uri = substr($uri, strlen($systemDir));
    if (substr($uri, 0, 1) != '/') {
        $uri = '/' . $uri;
    }
}
...
/**
 * 模板中再次导入模板，或在php代码中导入模板
 * @param $p
 */
function q_include_tpl($p)
{
    if (routermobile() && is_file(SystemRoot . TemplateDir . '/m/' . $p)) {
        $tempfile = ('m/' . $p);
    } else {
        $tempfile = $p;
    }

    return include_template($tempfile);
}
 ...
 * @return db|mixed
 */
function getDB($dbName = "", $dbType = "sqlite", $cs = array())
{
    if ($dbName) {
        ...
        if ($dbType == "sqlite") {
            ...
            $db_arr = array('database' => 'sqlitepdo', 'dbfile' => SystemRoot . '../data/' . $dbName . '.db');
        } else {
            if (!is_array($cs)) {
                $cs = array();
            }
            $db_arr = array('database' => 'mysqlpdo', 'host' => '', 'dbname' => '', 'user' => '', 'password' => '');
            $db_arr['host'] = isset($cs['host']) ? ($cs['host']) : "localhost:3306";
            $db_arr['user'] = isset($cs['user']) ? ($cs['user']) : "root";
            $db_arr['password'] = isset($cs['password']) ? ($cs['password']) : "123456";
            $db_arr['dbname'] = $dbName;
            $dnName_md5 = 'db_' . md5($db_arr['host'] . $dbName);
        }

        if (!isset($GLOBALS['dbs'][$dnName_md5])) {
            $GLOBALS['dbs'][$dnName_md5] = new db();
...
function u($cid = "", $key_array = null)
{
    if (empty($cid)) {
        Return gethomeurl();
    }
    Return getchannelurl($cid, $key_array = null);
}
...
//系统变量
function sys($strname, $return = 1)
{
    Return z($strname, $return);
}
...
//    var_dump($where);
    ...
    if (isset($where['page'])) {
        ...
        //$where['page'] = intval($where['page']);
        $where['pagecount'] = getDB()->fetchcount("SELECT count(id) FROM " . $where['tablename'] . " $sql");
...
//                $where['pageurl'] = $allpageurl[0];
    ...
    if (!isset($where['sql'])) {
        ...
        $idquery = getDB()->query($where['sql_id']);
        $ids = getDB()->fetchall($idquery);
        $sql = 'id in(';
        if (count($ids) > 0) {
...
//    echo $where['sql'];
    $query = getDB()->query($where['sql']);
    $articles = getDB()->fetchall($query);
    $i = 0;
    foreach ($articles as $value) {
        $i++;
        $value['key'] = $i;
        if (isset($where['stepstyle'])) {
            foreach ($where['stepstyle'] as $key => $val) {
                if ($i % $key == 0) {
                    $value['stepstyle'] = $where['stepstyle'][$key];
                }
            }
            if (!isset($value['stepstyle'])) {
                $value['stepstyle'] = '';
            }
        }
        if (isset($where['rowstyle'])) {
            foreach ($where['rowstyle'] as $key => $val) {
                if ($i == $key) {
                    $value['rowstyle'] = $where['rowstyle'][$key];
                }
            }
            if (!isset($value['rowstyle'])) {
                $value['rowstyle'] = '';
            }
        }
...
function getkinds($cid, $fid)
{
    ...
    $query = getDB()->query("SELECT id FROM $kindtable where fid='$fid';");
    $link = getDB()->fetchall($query);
...
function ainsert($article, $setting = '')
{
    ...
    if ($setting['check']) {
        $columns = getDB()->all("select mname,mkind,ifonly,strdefault,msetting from " . tableex('moudle') . " where cid=" . $setting['cid'] . "  order by morder asc;");
    ...
    $result = getDB()->insert($setting['tablename'], $article);
    if ($result) {
        Return $result;
    } else {
        Return false;
...
function aedit($article, $setting = '')
{
    ...
    if ($setting['check']) {
        $columns = getDB()->all("select mname,ifonly,strdefault,msetting from " . tableex('moudle') . " where cid=" . $setting['cid'] . "  order by morder asc;");
        foreach ($columns as $thiscolumn) {
            $key = $thiscolumn['mname'];
            if (isset($article[$key])) {
                $thismsetting = json_decode($thiscolumn['msetting'], 1);
    ...
    $result = getDB()->update($setting['tablename'], "id='$id'", $article);
    if ($result) {
        Return true;
    } else {
        Return false;
...
function adel($cid, $id, $tablename = '')
{
    ...
    $query = getDB()->query("delete from `$tablename` WHERE id='$id'");
    if ($query) {
        Return true;
    } else {
        Return false;
...
function acheck($mname, $value, $cid = '', $thismsetting = '', $id = 0)
{
    ...
    if ($thismsetting == '') {
        $column = getDB()->one("select mname,ifonly,msetting from " . tableex('moudle') . " where cid='" . $cid . "' and mname='" . $mname . "' limit 1;");
    ...
    if ($thismsetting['ifonly'] == 1) {
        ...
        if ($id > 0) {
            $pagecount = getDB()->fetchcount("SELECT id FROM " . $thismsetting['tablename'] . " where $mname='$value' and cid='" . $cid . "' and id<>'$id' limit 1;");
        } else {
            $pagecount = getDB()->fetchcount("SELECT id FROM " . $thismsetting['tablename'] . " where $mname='$value' and cid='" . $cid . "' limit 1;");
        }
        if ($pagecount) {
            Return 'exist';
        }
...
function getallchannelscache()
{
    ...
    if ($channelcache) {
        ...
        $channelslist = getDB()->all("SELECT cid,fid,ifshownav,cname,ckind,cvalue,newwindow,csetting FROM " . tableex('channel') . " where ckind<>'4' order by corder asc");
        if ($channelslist) {
            $channels = array();
...
function getchannelscache()
{
    ...
    if ($channelcache) {
        ...
        $channelslist = getDB()->all("SELECT cid,fid,ifshownav,cname,ckind,cvalue,newwindow,csetting FROM " . tableex('channel') . " where ckind<>'4' and ifshow=1 order by corder asc");
        if ($channelslist) {
            $channels = array();
            foreach ($channelslist as $val) {
                unset($thischannel);
...
function getchannelcache($cid)
{
    ...
    if ($channelcache) {
        ...
        if (is_numeric($cid)) {
            $channelcache = getDB()->one("SELECT * FROM " . tableex('channel') . " where cid='$cid' limit 1");
        } else {
            $channelcache = getDB()->one("SELECT * FROM " . tableex('channel') . " where cname='$cid' limit 1");
        }
        if ($channelcache) {
...
function getchannelurl($cid, $key_array = null)
{
    if (!is_array($cid)) {
...
function str($strname, $cid = '', $return = 1)
{
    ...
    $link = getDB()->one("SELECT strcid,strvalue FROM " . tableex('str') . " where strname='$strname' and strcid='$cid' limit 1;");
    if ($link) {
        $GLOBALS['strget'][$cid][$strname] = $link['strvalue'];
        if (SiteCache) {
            cacheset($cachehash, $link['strvalue'], 3600 * 24 * 365, 'str');
        }
        if ($return == 1) {
            Return $GLOBALS['strget'][$cid][$strname];
        } else {
...
function strset($strname, $strvalue, $cid = 0)
{
    ...
    $query = getDB()->query("UPDATE " . tableex('str') . " SET strvalue='$strvalue' WHERE strname='$strname' and strcid='$cid';");
    if ($query) {
...
function text2($str, $length = 10, $addtext)
{
    $str = nohtml($str);
    $cuttext = cut_str($str, $length);
    if (strlen($cuttext) < strlen($str)) {
        $cuttext .= $addtext;
    }
...
function session($keyname, $value = '7722914', $overtime = '31536000')
{
    if (SiteCache == false) {
        die('Cache closed');
    }
    if (isset($_COOKIE["QAPPSESSID"])) {
        $cookie = $_COOKIE["QAPPSESSID"];
    } else {
        $cookie = md5(md5(time()) . rand(10240000, 102400000));
        setcookie("QAPPSESSID", $cookie, time() + 31536000, "/");
        $_COOKIE["QAPPSESSID"] = $cookie;
    }
...
//            echo "pdo:" . getMillisecond() . "<br>";
            $dbfile = 'sqlite:' . $site_db['dbfile'];
//            echo "pdo:" . getMillisecond() . "<br>";
            $this->link = new PDO($dbfile);
//            echo $dbfile;
//            echo "pdo:" . getMillisecond() . "<br>";
        } elseif ($site_db['database'] == 'mysqlpdo') {
            if (!isset($site_db['dbname']) || empty($site_db['dbname'])) {
                $dbinfo = '';
            } else {
                $dbinfo = 'dbname=' . $site_db['dbname'];
            }
            $site_db['hostinfo'] = explode(':', $site_db['host']);
            if (count($site_db['hostinfo']) > 1) {
                $site_db['host'] = $site_db['hostinfo'][0];
                $site_db['port'] = $site_db['hostinfo'][1];
            } else {
                $site_db['port'] = '3306';
            }
            try {
...
function match_route()
{
    ...
    if ($GLOBALS['no_arg_uri'] == '/' . IndexFile) {
        $GLOBALS['no_arg_uri'] = '/';
    }
    ...
    foreach ($router as $routerkey => $routervalue) {
        ...
        $ifmatch = true;
        if (RouterDebug) {
            echo("<!-- Router: " . str_replace("\\/", "/", json_encode($routervalue)) . "-->\r\n");
        }
        if (isset($routervalue['domain'])) {
            $ifmatch = macthdomain($routervalue['domain']);
        }
        if ($ifmatch == false || (isset($routervalue['uri']) && matchuris($routervalue['uri'], $uri) === false)) {
            ...
            if (isset($routervalue['header'])) {
                routerheader($routervalue['header']);
            }
            ...
            if (isset($routervalue['cid'])) {
                routercid($routervalue['cid'], $ifmobile);
            }
...
function get_router()
{
    ...
    if ($router) {
        ...
        $routerquery = getDB()->one("SELECT strvalue FROM " . tableex('str') . " where strname='router' and inputkind=0 limit 1;");
...
function init_route($routerkey, $ifmobile)
{
    ...
    if (!isset($router[$routerkey]) && $routerkey !== 'not_found') {
        ...
        echo(base64_decode(""));
    ...
    if (isset($router[$routerkey]['res'])) {
        ...
        foreach ($ress as $thisres) {
            ...
            if (!isset($GLOBALS['included'][$thistemplatefile]) && !empty($thistemplatefile)) {
                ...
                if ($ifmobile && !strpos("route.php", $thistemplatefile) && is_file(SystemRoot . TemplateDir . DIRECTORY_SEPARATOR . 'm' . DIRECTORY_SEPARATOR . $thistemplatefile)) {
            ...
            if ($thistemplatefunction && is_callable($thistemplatefunction)) {
...
function macthdomain($domains)
{
    ...
    foreach ($domains as $val) {
        ...
        if ($thisstripos === false) {
            ...
            if (substr(server_name(), -strlen($topdomain)) == $topdomain) {
                Return true;
            }
        }
    }
...
{
    if (SiteCache == false && TemplateTime > -1) {
        die('Cache closed!');
    }
...
function ucms_template($file)
{
    ...
    $system_var[2] = array(
        ...
        gethomeurl(), "\r\n",
        base64_decode('PGEgaHJlZj0iaHR0cDovL2p4c2JveC5jb20iIHRhcmdldD0iX2JsYW5rIj5KWFNCT1guQ09NPC9hPuKAi+KAiw==')
    );
    $system_syntax = array();
    ...
    if (!defined('TemplateStart')) {
        define('TemplateStart', '{');
    }
    if (!defined('TemplateEnd')) {
        define('TemplateEnd', '}');
    }
            ...
            if ($templist[3][$key]) {
                ...
                foreach ($everyfunction as $function_key => $thisfunction) {
                    ...
                    if (isset($iffunction[1][0])) {
                        ...
                        if ($function_key > 0 && !in_array('this', $this_function_args)) {
                            $this_function_args_new[] = 'this';
                        }
    ...
    foreach ($admin_hooks as $thishook) {
        if ($cid == $thishook['cid']) {
            ...
            foreach ($thishook_actions as $thishook_action) {
                if ($action == $thishook_action || $thishook_action == "*") {
                    runfunc($thishook['funcname'], $action, $cid);
                }
            }
        }
    }
...
function sys_hook($pg, $action, $funcname)
{
    global $sys_hooks;
    if (is_array($pg)) {
        foreach ($pg as $key => $val) {
            $sys_hooks[] = array('page' => $val, 'action' => $action, 'funcname' => $funcname);
        }
    } else {
        $sys_hooks[] = array('page' => $pg, 'action' => $action, 'funcname' => $funcname);
    }
    Return true;
}

function run_sys_hook($pg, $action)
{
    global $sys_hooks;
    foreach ($sys_hooks as $thishook) {
        if ($pg == $thishook['page']) {
            $thishook_actions = explode(';', $thishook['action']);
            foreach ($thishook_actions as $thishook_action) {
                if ($action == $thishook_action || $thishook_action == "*") {
                    runfunc($thishook['funcname'], $action, $pg);
                }
            }
        }
    }
...
function web_hook($pg, $action, $funcname)
{
//    jLog("webhook", "注册webhook:" . $pg . "--" . $action . '--' . $funcname);
    global $web_hooks;
    if (is_array($pg)) {
        foreach ($pg as $key => $val) {
            $web_hooks[] = array('page' => $val, 'action' => $action, 'funcname' => $funcname);
        }
    } else {
        $web_hooks[] = array('page' => $pg, 'action' => $action, 'funcname' => $funcname);
    }
    Return true;
}

function run_web_hook($pg, $action)
{
//    jLog("run_web_hook",$pg."--".$action);
//    jLog("run_web_hook", "运行webhook:" . $pg . "--" . $action);

    global $web_hooks;
    foreach ($web_hooks as $thishook) {
        if ($pg == $thishook['page']) {
            $thishook_actions = explode(';', $thishook['action']);
            foreach ($thishook_actions as $thishook_action) {
                if ($action == $thishook_action || $thishook_action == "*") {
